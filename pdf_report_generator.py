...
class SecurityReportPDF:
    ...
    def add_dangerous_functions_section(self, security_analysis):
        ...
        for lang, funcs in list(dangerous_by_lang.items())[:5]:  # Limit to 5 languages
            ...
            for func in funcs[:15]:  # Limit to 15 per language
                file_name = os.path.basename(func['file'])
                table_data.append([
                    func['function'][:30],
                    func['category'][:20],
                    file_name[:30],
    ...
    def add_taint_flows_section(self, taint_flows):
        ...
        for i, flow in enumerate(taint_flows[:20], 1):  # Limit to 20
            flow_data = [
                ['Flow #', str(i)],
                ['Risk Level', flow['risk']],
                ['File', os.path.basename(flow['file'])],
                ['Source', f"{flow['source']} (line {flow['source_line']})"],
                ['Sink', f"{flow['sink']} (line {flow['sink_line']})"],
                ['Description', flow['description'][:100]]
            ]
    ...
    def add_secrets_section(self, security_analysis):
        ...
        for secret in all_secrets[:20]:
            ...
            value_preview = secret['value'][:40] + "..." if len(secret['value']) > 40 else secret['value']
    ...
    def generate(self, analysis_result, project_name=""):
        ...
        self.add_framework_findings_section(
            analysis_result.get('framework_security_findings', [])
        )
